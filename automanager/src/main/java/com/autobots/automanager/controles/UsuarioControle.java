...
public class UsuarioControle {
	...
	public ResponseEntity<List<Usuario>> pegarTodos(){
		...
		if(todos.isEmpty()) {
			...
			ResponseEntity<List<Usuario>> resposta = new ResponseEntity<List<Usuario>>(todos, status);
			return resposta;
		}
	}
	...
	public ResponseEntity<?> atualizarUsuario(@PathVariable Long id, @RequestBody Usuario atualizador){
	...
	public ResponseEntity<?> cadastrarUsuario(@RequestBody Usuario cadastro){
		...
		if(cadastro.getEmails().isEmpty()) {
			return new ResponseEntity<>("Por Favor, coloque seu E-mail", HttpStatus.NOT_ACCEPTABLE);
		}else {
			if(cadastro.getDocumentos().isEmpty()) {
				return new ResponseEntity<>("Por favor, Coloque um Documento", HttpStatus.NOT_ACCEPTABLE);
			}else {
					...
					if(EmailExistente == true) {
						...
						if(cadastro.getPerfis().toString().contains("FORNECEDOR")) {
							if(cadastro.getMercadorias().isEmpty()) {
								return new ResponseEntity<>("Fornecedor encontrado sem mercadoria, por favor insira um", HttpStatus.NOT_ACCEPTABLE);
							}
							else {
								for(Mercadoria mercadoria : cadastro.getMercadorias()) {
